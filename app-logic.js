...
// ✅ New Firebase project connection
var firebaseConfig = {
  apiKey: "AIzaSyDya1dDRSeQmuKnpraSoSoTjauLlJ_J94I",
  authDomain: "proglove-bowl-tracker.firebaseapp.com",
  databaseURL: "https://proglove-bowl-tracker-default-rtdb.firebaseio.com", // <== Make sure this matches your Firebase console
...
function initFirebaseAndStart() {
  try {
    if (typeof firebase === 'undefined' || !firebase.apps) {
      updateSystemStatus(false, "⚠️ Firebase SDK not loaded - using local data");
      loadFromLocal();
      initializeUI();
      return;
    }

    // Delete any old cached Firebase apps before initializing
    if (firebase.apps.length > 0) {
      console.log("🔄 Removing old Firebase apps...");
  ...
  try {
    ...
    connectedRef.on("value", function (snap) {
      if (snap && snap.val() === true) {
        updateSystemStatus(true, "✅ Connected to Firebase");
      } else {
        updateSystemStatus(false, "⚠️ Firebase Disconnected");
      }
    });
  } catch (e) {
    console.warn("⚠️ monitorConnection failed:", e);
...
function loadFromFirebase() {
  try {
    var db = firebase.database();
    var ref = db.ref('progloveData'); // <-- Your main data node
    updateSystemStatus(false, '🔄 Loading data from cloud...');
    ref.once('value').then(function (snapshot) {
      if (snapshot && snapshot.exists && snapshot.exists()) {
        ...
        showMessage("✅ Data successfully loaded from new Firebase project", "success");
      } else {
        updateSystemStatus(true, "✅ Connected (no data found)");
        loadFromLocal();
      }
      initializeUI();
    }).catch(function (err) {
